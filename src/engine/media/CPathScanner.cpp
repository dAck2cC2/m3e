...
void CPathScanner::setLocale(const char *locale)
{
    if (mLocale) {
        free(mLocale);
        mLocale = NULL;
...
const char *CPathScanner::locale() const
{
    return mLocale;
}
...
int
CPathScanner::processDirectory(const char *path, sp<IEngineScannerClient> &client)
{
    if (client == 0) {
    ...
    int result = doProcessDirectory(pathBuffer, pathRemaining, client, false);
...
bool CPathScanner::shouldSkipDirectory(char *path)
{
...
int
CPathScanner::doProcessDirectory(
    char *path, int pathRemaining, sp<IEngineScannerClient>& client, bool noMedia)
{
    ...
    int result = MEDIA_SCAN_RESULT_OK;
...
int
CPathScanner::doProcessDirectoryEntry(
    char *path, int pathRemaining, sp<IEngineScannerClient>& client, bool noMedia,
