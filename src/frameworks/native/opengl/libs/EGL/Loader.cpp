...
#if defined(_MSC_VER)
	void* so = LoadLibrary(path);
	ALOGE_IF(!so, "dlopen(\"%s\") failed: %d", path, GetLastError());
#else
    ...
    while (*api) {
...
#if defined(_MSC_VER) 
		(__eglMustCastToProperFunctionPointerType)GetProcAddress((HMODULE)dso, name);
#else
        ...
        if (f == NULL) {
            ...
            if ((index>0 && (index<SIZE-1)) && (!strcmp(name+index, "OES"))) {
                ...
                scrap[index] = 0;
#if defined(_MSC_VER) 
				f = (__eglMustCastToProperFunctionPointerType)GetProcAddress((HMODULE)dso, scrap);
#else
        ...
        egl_connection_t* cnx, uint32_t mask)
{
...
#if defined(_MSC_VER)
	void* dso = LoadLibrary(driver_absolute_path);
	if (dso == 0) {
		DWORD err = GetLastError();
		ALOGE("load_driver(%s): %d", driver_absolute_path, err);
		return 0;
	}
#else
    ...
    if (mask & EGL) {
#if defined(_MSC_VER) 
		getProcAddress = (getProcAddressType)GetProcAddress((HMODULE)dso, "eglGetProcAddress");
#else
