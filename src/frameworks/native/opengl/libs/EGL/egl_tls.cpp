...
thread_store_t egl_tls_t::sKey = THREAD_STORE_INITIALIZER;
...
#else
	if (!(sKey.has_tls)) {
		thread_store_set(&sKey, NULL, (void(*)(void*))&eglReleaseThread);
	}
#endif
}
...
egl_tls_t* egl_tls_t::getTLS() {
    egl_tls_t* tls = (egl_tls_t*)thread_store_get(&sKey);
    if (tls == 0) {
        tls = new egl_tls_t;
		thread_store_set(&sKey, tls, (void(*)(void*))&eglReleaseThread);
    }
    return tls;
}
...
void egl_tls_t::clearTLS() {
    if (sKey.has_tls) {
        egl_tls_t* tls = (egl_tls_t*)thread_store_get(&sKey);
        if (tls) {
			thread_store_set(&sKey, 0, (void(*)(void*))&eglReleaseThread);
            delete tls;
        }
    }
}
