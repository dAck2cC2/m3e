...
static mutex_t sExtensionMapMutex = MUTEX_INITIALIZER;
...
EGLBoolean KHRONOS_APIENTRY eglGetConfigs(   EGLDisplay dpy,
                            EGLConfig *configs,
...
EGLBoolean KHRONOS_APIENTRY eglChooseConfig( EGLDisplay dpy, const EGLint *attrib_list,
...
EGLSurface KHRONOS_APIENTRY eglCreateWindowSurface(  EGLDisplay dpy, EGLConfig config,
                                    ...
                                    const EGLint *attrib_list)
{
    ...
    if (dp) {
        ...
        int result = native_window_api_connect((struct ANativeWindow*)window, NATIVE_WINDOW_API_EGL);
        if (result != OK) {
        ...
        if (format != 0) {
            int err = native_window_set_buffers_format((struct ANativeWindow*)window, format);
            if (err != 0) {
                ALOGE("error setting native window pixel format: %s (%d)",
                        strerror(-err), err);
                native_window_api_disconnect((struct ANativeWindow*)window, NATIVE_WINDOW_API_EGL);
                return setError(EGL_BAD_NATIVE_WINDOW, EGL_NO_SURFACE);
            }
        }

        if (dataSpace != 0) {
            int err = native_window_set_buffers_data_space((struct ANativeWindow*)window, dataSpace);
            if (err != 0) {
        ...
        native_window_set_buffers_format((struct ANativeWindow*)window, 0);
        native_window_api_disconnect((struct ANativeWindow*)window, NATIVE_WINDOW_API_EGL);
    }
    return EGL_NO_SURFACE;
}

EGLSurface KHRONOS_APIENTRY eglCreatePixmapSurface(  EGLDisplay dpy, EGLConfig config,
...
EGLSurface KHRONOS_APIENTRY eglCreatePbufferSurface( EGLDisplay dpy, EGLConfig config,
                                    const EGLint *attrib_list)
{
    clearError();
...
EGLBoolean KHRONOS_APIENTRY eglQuerySurface( EGLDisplay dpy, EGLSurface surface,
                            EGLint attribute, EGLint *value)
{
    clearError();
...
EGLContext KHRONOS_APIENTRY eglCreateContext(EGLDisplay dpy, EGLConfig config,
...
EGLBoolean KHRONOS_APIENTRY eglQueryContext( EGLDisplay dpy, EGLContext ctx,
                            EGLint attribute, EGLint *value)
{
    clearError();
...
#if defined(_MSC_VER) 
	proc = GetProcAddress((HMODULE)cnx->libEgl, procname);
	if (proc) return (__eglMustCastToProperFunctionPointerType)proc;

	proc = GetProcAddress((HMODULE)cnx->libGles2, procname);
	if (proc) return (__eglMustCastToProperFunctionPointerType)proc;

	proc = GetProcAddress((HMODULE)cnx->libGles1, procname);
	if (proc) return (__eglMustCastToProperFunctionPointerType)proc;
#else
...
__eglMustCastToProperFunctionPointerType EGLAPIENTRY eglGetProcAddress(const char *procname)
{
	...
	mutex_lock(&sExtensionMapMutex);
		...
		mutex_unlock(&sExtensionMapMutex);
    return addr;
}
...
EGLBoolean KHRONOS_APIENTRY eglCopyBuffers(  EGLDisplay dpy, EGLSurface surface,
...
EGLBoolean KHRONOS_APIENTRY eglSurfaceAttrib(
...
EGLBoolean KHRONOS_APIENTRY eglBindTexImage(
...
EGLBoolean KHRONOS_APIENTRY eglReleaseTexImage(
        EGLDisplay dpy, EGLSurface surface, EGLint buffer)
{
    clearError();
...
EGLSurface KHRONOS_APIENTRY eglCreatePbufferFromClientBuffer(
