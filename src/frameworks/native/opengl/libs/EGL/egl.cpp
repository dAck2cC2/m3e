...
void setGLHooksThreadSpecific(gl_hooks_t const *value) {
	...
	thread_store_set(&gTLS, (void *)value, threadDestructor);
}

gl_hooks_t const* getGlThreadSpecific()
{
	return (gl_hooks_t const*)(thread_store_get(&gTLS));
}
...
static int sEarlyInitState = 0; // pthread_once(&once_control, &early_egl_init);
...
static mutex_t sInitDriverMutex = MUTEX_INITIALIZER;

EGLBoolean egl_init_drivers() {
    EGLBoolean res;
	mutex_lock(&sInitDriverMutex);
    res = egl_init_drivers_locked();
	mutex_unlock(&sInitDriverMutex);
    return res;
}

static mutex_t sLogPrintMutex = MUTEX_INITIALIZER;
...
void gl_unimplemented() {
	...
	mutex_lock(&sLogPrintMutex);
    if ((now - sLogPrintTime) > NSECS_DURATION) {
        sLogPrintTime = now;
        printLog = true;
    }
	mutex_unlock(&sLogPrintMutex);
    if (printLog) {
