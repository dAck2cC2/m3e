...
* customized
*****************************************************************************/
void* HWComposer::nativeWindowMain()
{
	if (mHwc && mHwc->nativeWindowMain) {
		return ((*(mHwc->nativeWindowMain))());
	}
	else {
		return NULL;
	}
...
int  HWComposer::nativeWindowPopEvent()
{
	if (mHwc && mHwc->nativeWindowPopEvent) {
		return ((*(mHwc->nativeWindowPopEvent))());
	}
	else {
		return -1;
	}
...
void* HWComposer::nativeWindowCreate(const char* name)
{
	if (mHwc && mHwc->nativeWindowCreate) {
		return ((*(mHwc->nativeWindowCreate))(name));
	}
	else {
		return NULL;
	}
...
void HWComposer::nativeWindowDelete(void* hWin)
{
	if (mHwc && mHwc->nativeWindowDelete) {
		((*(mHwc->nativeWindowDelete))(hWin));
	}

	return;
}

void  HWComposer::nativeWindowMessageLoop(void* hWin)
{
	if (mHwc && mHwc->nativeWindowMessageLoop) {
		((*(mHwc->nativeWindowMessageLoop))(hWin));
	}
