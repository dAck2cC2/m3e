...
/* M3E: avoid too many logs from Pacel */
#define LOG_NDEBUG 1
...
/* M3E: define SIZE_T_MAX */
#ifndef SIZE_T_MAX
#include <limits.h>
#define SIZE_T_MAX  SIZE_MAX
#endif // M3E

#if defined(_MSC_VER) // M3E: evil interface on MSVC
#ifdef interface
#undef interface
#endif // interface
#endif // M3E
...
#if !defined(_MSC_VER) // M3E: no ashmem
...
#if !defined(_MSC_VER) // M3E: no fcntl
...
status_t Parcel::restartWrite(size_t desired)
{
    ...
    /* M3E: crashing when empty mObjects */
    if (mObjects) {
    free(mObjects);
    mObjects = nullptr;
    }
...
void Parcel::Blob::release() {
    if (mFd != -1 && mData) {
        //::munmap(mData, mSize); /* M3E: no mmap */
    }
    clear();
}
