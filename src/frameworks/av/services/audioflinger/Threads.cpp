    ...
    for (;;) {
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            for (;;) {
                ...
                LOG_ALWAYS_FATAL_IF((status == OK) != (framesOut > 0),"_MSC_VER"); // M3E:
        ...
        AudioBufferProvider::Buffer* buffer)
{
    ...
    LOG_ALWAYS_FATAL_IF(!(0 <= filled && (size_t) filled <= recordThread->mRsmpInFrames), "_MSC_VER"); // M3E:
