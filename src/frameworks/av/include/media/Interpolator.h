...
public:
    ...
    status_t writeToParcel(Parcel *parcel) const {
        ...
        status_t res = parcel->writeInt32(mInterpolatorType)
                ? -1 : parcel->writeFloat(mFirstSlope)
                ? -2 : parcel->writeFloat(mLastSlope)
                ? -3 : parcel->writeUint32((uint32_t)this->size()); // silent truncation
        if (res != NO_ERROR) {
            return res;
        }
        for (const auto &pt : *this) {
            res = parcel->writeFloat(pt.first)
                    ? -1 : parcel->writeFloat(pt.second);
            if (res != NO_ERROR) {
                return res;
            }
        }
    ...
    status_t readFromParcel(const Parcel &parcel) {
        ...
        status_t res = parcel.readInt32(&type)
                        ? -1 : parcel.readFloat(&mFirstSlope)
                        ? -2 : parcel.readFloat(&mLastSlope)
                        ? -3 : parcel.readUint32(&size)
                        ? -4 : setInterpolatorType((InterpolatorType)type);
        if (res != NO_ERROR) {
            return res;
        }
