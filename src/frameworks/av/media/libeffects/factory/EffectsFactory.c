...
static mutex_t gLibLock = MUTEX_INITIALIZER; // controls access to gLibraryList
...
int Effect_Process(effect_handle_t self, audio_buffer_t *inBuffer, audio_buffer_t *outBuffer)
{
    ...
    mutex_lock(&gLibLock);
    if (fx->lib == NULL) {
        mutex_unlock(&gLibLock);
        return -EPIPE;
    }
    mutex_lock(&fx->lib->lock);
    mutex_unlock(&gLibLock);

    ret = (*fx->subItfe)->process(fx->subItfe, inBuffer, outBuffer);
    mutex_unlock(&fx->lib->lock);
    return ret;
}
...
#if defined(_MSC_VER) 
	desc = (audio_effect_library_t *)GetProcAddress((HMODULE)hdl, AUDIO_EFFECT_LIBRARY_INFO_SYM_AS_STR);
#else
