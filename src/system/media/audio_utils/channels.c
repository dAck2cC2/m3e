...
static size_t contract_channels_non_destructive(const void* in_buff, size_t in_buff_chans,
                                ...
                                unsigned sample_size_in_bytes, size_t num_in_bytes)
{
#if defined(_MSC_VER) // M3E: CONTRACT_CHANNELS_NON_DESTRUCTIVE doesn't build on MSVC, implement it when it is needed
    //LOG_ALWAYS_FATAL_IF("contract_channels_non_destructive");
    return 0;
#else  // M3E
    switch (sample_size_in_bytes) {
    case 1:
...
{
#if defined(_MSC_VER) // M3E: CONTRACT_CHANNELS_NON_DESTRUCTIVE doesn't build on MSVC, implement it when it is needed
    //LOG_ALWAYS_FATAL_IF("expand_channels_non_destructive");
    return 0;
#else  // M3E
    switch (sample_size_in_bytes) {
    case 1:
