...
static mutex_t lock_loggable = MUTEX_INITIALIZER;
...
static void unlock()
{
    mutex_unlock(&lock_loggable);
}

struct cache {
    const struct prop_info *pinfo;
    uint32_t serial;
    unsigned char c;
};
...
static int __android_log_level(const char *tag, int default_prio)
{
    ...
    char key[PROP_NAME_MAX * 2]; /* may be > PROPERTY_KEY_MAX */
    char *kp;
    size_t i;
    char c = 0;
    /*
