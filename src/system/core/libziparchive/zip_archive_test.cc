...
#if defined(_MSC_VER) // M3E: separator
static std::string test_data_dir = android::base::GetExecutableDirectory() + "\\testdata";
#else  // M3E
...
TEST(ziparchive, OpenAssumeFdOwnership) {
  ...
  CloseArchive(handle);
#if !defined(_MSC_VER) // M3E: crash on MSVC
