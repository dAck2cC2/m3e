...
#include "media/MediaBufferGroup.h"
...
WAVExtractor::WAVExtractor(const sp<IDataSource> &source)
    : mDataSource(source),
      mValidFormat(false),
    ...
    while (remainingSize >= 8) {
        ...
        if (!memcmp(chunkHeader, "fmt ", 4)) {
            ...
            if (mValidFormat) {
                ...
                mTrackMeta->setInt32(kKeyDataSize, mDataSize);
#endif // ENABLE_CUSTOMISE
...
bool SniffWAV(
    const sp<IDataSource> &source, String8 *mimeType, float *confidence
